/**
 * Use the `avalon_keychain` methods listed below to issue requests to the avalon blockchain.
 */
var avalon_keychain = {
  current_id: 1,
  requests: {},
  ...
  requestHandshake: function (callback) {
    ...
    this.dispatchCustomEvent("swHandshake_avalon", "");
  },
   ...
   * @param {String} username avalon account to perform the request
  ...
  requestEncodeMessage: function (
    ...
    this.dispatchCustomEvent("swRequest_avalon", request, callback);
  },
  /**
   * This function is called to verify that the user has a certain authority over an account, by requesting to decode a message
   * @param {String} account avalon account to perform the request
   ...
   * @param {String} [account=null] avalon account to perform the request. If null, user can choose the account from a dropdown
   ...
   * @param {Object} comment_options Options attached to the blog post. Consult avalon documentation to learn more about it
   * @param {function} callback Keychain's response to the request
   * @param {String} [rpc=null] Override user's RPC settings
   */
   ...
   * Requests a transfer
   * @param {String} account avalon account to perform the request
   * @param {String} to avalon account to receive the transfer
   ...
   * @param {String} currency 'avalon' or 'HBD'
   * @param {function} callback Keychain's response to the request
   ...
   * @param {String} avalon Amount of avalon to be powered up
   * @param {function} callback Keychain's response to the request
   * @param {String} [rpc=null] Override user's RPC settings
   */
  requestPowerUp: function (username, recipient, avalon, callback, rpc) {
    var request = {
      type: "powerUp",
      username,
      recipient,
   ...
   * @param {String} username avalon account to perform the request
   * @param {String} avalon_power Amount of avalon to be powered down
   * @param {function} callback Keychain's response to the request
   * @param {String} [rpc=null] Override user's RPC settings
   */
  requestPowerDown: function (username, avalon_power, callback, rpc) {
    var request = {
      type: "powerDown",
      username,
      steem_power: avalon_power,
      rpc,
    };
    this.dispatchCustomEvent("swRequest_avalon", request, callback);
  },
   ...
   * @param {Boolean} collaterized true to convert avalon to HBD. false to convert HBD to avalon.
   * @param {function} callback Keychain's response to the request
   * @param {String} [rpc=null] Override user's RPC settings
   */
   ...
   * @param {String} [username=null] avalon account to perform the request
   * @param {String} to avalon account receiving the transfers.
   * @param {String} amount amount to be sent on each execution.
   * @param {String} currency avalon or HBD on mainnet.
...
window.addEventListener(
  ...
  function (event) {
    ...
    if (event.data.type && event.data.type == "avalon_keychain_response") {
      const response = event.data.response;
      if (response && response.request_id) {
        if (avalon_keychain.requests[response.request_id]) {
          avalon_keychain.requests[response.request_id](response);
          delete avalon_keychain.requests[response.request_id];
        }
      }
    } else if (
      event.data.type &&
      event.data.type == "avalon_keychain_handshake"
    ) {
      if (avalon_keychain.handshake_callback) {
        avalon_keychain.handshake_callback();
      }
    }
  },
  false
